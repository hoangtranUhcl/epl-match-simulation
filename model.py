...
# Aggregate player values per team per season
...
le = LabelEncoder()
match_results_features['result'] = le.fit_transform(match_results_features['result'] + 1)  # Shift to 0, 1, 2 (Away, Draw, Home)
...
# Make predictions
fixture_predictions = model.predict(X_fixtures)
fixture_predictions_proba = model.predict_proba(X_fixtures)

# Create results DataFrame
results = fixtures[['fixture_date', 'home_team_id', 'away_team_id']].copy()
results['predicted_result'] = le.inverse_transform(fixture_predictions) - 1  # Shift back to -1, 0, 1
results['confidence'] = np.max(fixture_predictions_proba, axis=1) * 100  # Max probability as confidence %
...
results = results[['fixture_date', 'home_team_name', 'away_team_name', 'winner', 'confidence']]

# 8. Display Results
print("\nUpcoming Match Predictions (Starting Feb 25, 2025):")
